...
class RxPermissions
{
    @VisibleForTesting var mRxPermissionsFragment: Lazy<RxPermissionsFragment>
    ...
    {
        return ObservableTransformer<T, Permission> {
                o ->
            request(o, *permissions).buffer(permissions.size)
                .flatMap(Function<List<Permission>, ObservableSource<Permission>> {
                        permissions ->
                    if (permissions.isEmpty())
                    {
                        Observable.empty()
    ...
    private fun requestImplementation(vararg permissions: String): Observable<Permission?>
    {
        ...
        val unrequestedPermissions: MutableList<String> =
            ArrayList() // In case of multiple permissions, we create an Observable for each of them.
        ...
        for (permission in permissions)
        {
            ...
            var subject = mRxPermissionsFragment.get()
                .getSubjectByPermission(permission) // Create a new subject if not exists
            if (subject == null)
            {
