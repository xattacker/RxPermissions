...
class RxPermissionsFragment : Fragment()
{
    ...
    )
    {
        ...
        while (i < size)
        {
            log("onRequestPermissionsResult  " + permissions[i]) // Find the corresponding subject
            val subject: PublishSubject<Permission>? = mSubjects[permissions[i]]
            if (subject == null)
            { // No subject found
                Log.e(
                    RxPermissions.Companion.TAG,
    ...
    fun log(message: String)
    {
        if (mLogging)
        {
            Log.d(RxPermissions.Companion.TAG, message)
        }
    }
